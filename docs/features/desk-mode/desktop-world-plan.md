...
这是 Desk 的通用模型接入流程，不是 MINGTU 电脑的特例。以后新增或替换任何桌面模型，
默认按下面的顺序处理：

1. 将原始 FBX、GLB 或 glTF 放在本地 `assets/3d/` 素材目录，保留源文件，不直接复制到
   页面组件中。
2. 用 `desk:process-model` 生成对应的 `public/desk/models/<name>.glb`；需要调整单位或
   朝向时，只通过 `--scale`、`--rotation` 显式记录，而不是在 React 场景里补偿。
3. 检查处理器输出的网格数量、尺寸、底部中心原点和 `missingTextures`，确认生成文件是
   非空的自包含 GLB。
4. 在 `desk-canvas.tsx` 中只配置模型在桌面上的位置、尺寸和点击目标，不再引入
   `FBXLoader` 或为单个模型增加专属坐标修复。
5. 重新执行 Worker build，并用 `/desk` 的桌面端和移动端预览确认模型落点、材质和交互。
